...
export async function checkLoggedInUserApi() {
    const res = await defaultApiClient.get<{ user: IUser; backendTokens: IBackendTokens }>("/v1/auth/logged_in");
    return res;
}
...
export async function refreshTokenApi(refreshToken: string) {
    const res = await defaultApiClient.post<undefined, IBackendTokens>("/v1/auth/refresh", undefined, {
        headers: {
